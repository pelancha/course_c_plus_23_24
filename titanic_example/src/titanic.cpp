...
void printMatrix(const stringMatrix& matrix)
{
...
void fillRate (stringMatrix& matrix)
{
    ...
    for (size_t i = 1; i < matrix.size(); i++)
    {
        ...
        double rate = defineRate(std::stoi(matrix[i][CSV_FIELDS::Age]), 
                                ...
                                matrix[i][CSV_FIELDS::Sex] == "male" ? 0: 1); //std::stoi - string to integer
...
void writeCsv(const std::string& filePath, const stringMatrix& matrix)
{
    ...
    for (const std::vector<std::string>& row: matrix) //работаем с оригиналом а не с копией
    {
